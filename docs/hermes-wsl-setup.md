...
在 Windows 上使用 Token Mate 时，能否检测到 AI 工具取决于数据的存放位置：

| 工具 | 数据位置 | Token Mate 检测方式 | 状态 |
|:---|:---|:---|:---:|
...
Hermes Agent 运行在 WSL 中，其会话数据存储在 `~/.hermes/state.db`（SQLite 数据库）。Token Mate 通过 `tokscale` 工具读取该数据库，但由于 Windows 侧通过 `\\wsl$` UNC 路径访问 WSL 内的 SQLite 文件存在兼容性问题（字节范围锁定、路径混合斜杠等），导致直接扫描无法读取数据。

## 解决方案

采用 Token Mate 的多设备架构：**WSL 内运行 headless agent → Windows 端 host hub 接收数据**。

```
┌─────────────────────────────────────────────────────────┐
│                    Windows                               │
│  ┌──────────────────────────────────────────────────┐   │
│  │  Token Mate Widget (Hub 主机模式)              │   │
│  │  ├── 本地扫描 → Codex ✅                          │   │
│  │  └── 接收 WSL agent → Hermes ✅                   │   │
│  └──────────────────────────────────────────────────┘   │
│                          ↑                               │
...
### 第二步：在 WSL 安装 Token Mate

如果你已通过安装包在 Windows 安装了 Token Mate，WSL 端只需安装 headless agent 所需的依赖：
...
```
Token Mate agent device=hermes-wsl hub=http://192.168.x.x:17321
...
```cron
*/30 * * * * cd /home/用户名/token-mate && no_proxy=192.168.x.x NO_PROXY=192.168.x.x npm run agent -- --once >> /tmp/token-mate-agent.log 2>&1
```

#### 方式 B：Systemd 用户服务

创建 `~/.config/systemd/user/token-mate-agent.service`：

```ini
[Unit]
Description=Token Mate Headless Agent (WSL)

[Service]
Type=oneshot
ExecStart=/usr/bin/npm run agent -- --once
WorkingDirectory=/home/用户名/token-mate
...
```ini
[Unit]
Description=Token Mate Agent Timer
...
systemctl --user enable --now token-mate-agent.timer
```
