...
<p align="center">
    <a href="https://github.com/atukunare/token-mate/releases"><img src="https://img.shields.io/github/v/release/atukunare/token-mate?include_prereleases&style=flat-square&label=release&color=22c55e" alt="最新發布" /></a>
    <a href="https://github.com/atukunare/token-mate/releases"><img src="https://img.shields.io/github/downloads/atukunare/token-mate/total?style=flat-square&color=22c55e" alt="總下載量" /></a>
...
Token Mate 是一款面向多工具 AI 編程開發者的本地優先桌面小工具。它在一個介面裡彙總即時 Token 用量、估算成本、帳戶用量上限與 session 明細，可在 macOS 選單列或 Windows 系統匣隨時查看。

每台裝置只讀取本機工具紀錄。你也可以透過自託管 hub 或 Cloudflare Worker 連接多台機器，在裝置之間同步摘要資料，而無需把提示詞、原始碼或憑證傳送到第三方。

由 [@atukunare](https://github.com/atukunare) 維護。

## 支援的工具

Token Mate 對「Token 用量」「帳戶用量上限」與「session 明細」分別支援：
...
大多數用量監控工具只在它執行的那台機器上有用。Token Mate 是為多裝置工作流而設計的：每台裝置監看自己的本機紀錄、把摘要更新送到你的 hub，每個連線中的小工具幾乎都能即時看到 Token 變化。
...
只要 Token Mate 還在跑，hub 就會運作——結束 App（僅關閉視窗不算）會停掉 hub，所有連入的裝置都會中斷。
...
你可以從 [release 頁面](https://github.com/atukunare/token-mate/releases) 下載 App。所有 release 都未簽章，發布說明含 macOS（arm64）與 Windows（x64）的首次啟動解鎖步驟。其他平台請從原始碼 `npm start` 啟動。
