...
<p align="center">
    <a href="https://github.com/atukunare/token-mate/releases"><img src="https://img.shields.io/github/v/release/atukunare/token-mate?include_prereleases&style=flat-square&label=release&color=22c55e" alt="最新发布" /></a>
    <a href="https://github.com/atukunare/token-mate/releases"><img src="https://img.shields.io/github/downloads/atukunare/token-mate/total?style=flat-square&color=22c55e" alt="总下载量" /></a>
...
Token Mate 是一款面向多工具 AI 编程开发者的本地优先桌面组件。它在一个界面里汇总实时 Token 用量、估算成本、账户用量上限与 session 明细，可在 macOS 菜单栏或 Windows 系统托盘随时查看。

每台设备只读取本机工具日志。你也可以通过自托管 hub 或 Cloudflare Worker 连接多台机器，在设备之间同步汇总数据，而无需把提示词、源码或凭证发送到第三方。

由 [@atukunare](https://github.com/atukunare) 维护。

## 支持的工具

Token Mate 对「Token 用量」「账户用量上限」和「session 明细」分别支持：
...
大多数用量监控工具只在它运行的那台机器上有用。Token Mate 是为多设备工作流而设计的：每台设备监视自己的本地日志、把汇总更新发送到你的 hub，每个连接中的小部件几乎都能实时看到 Token 变化。
...
只要 Token Mate 还在跑，hub 就会运行——退出 App（仅关闭窗口不算）会停掉 hub，所有连入的设备都会断开。
...
[![部署到 Cloudflare](https://deploy.workers.cloudflare.com/button)](https://deploy.workers.cloudflare.com/?url=https://github.com/atukunare/token-mate/tree/main/worker)
...
你可以从 [releases 页面](https://github.com/atukunare/token-mate/releases) 下载 App。所有 release 都未签名，发布说明含 macOS（arm64）与 Windows（x64）的首次启动解锁步骤。其他平台请从源码 `npm start` 运行。
